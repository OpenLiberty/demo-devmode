...
2. Start dev mode:
    ...
    Notice that a new file was automatically created at `src/main/liberty/config/configDropins/overrides/generated-features.xml` containing the cdi-2.0, mpMetrics-3.0 and jaxrs-2.1 features.
    ```xml
...
5. Delete the src/main/java/io/openliberty/sample/system/SystemLivenessCheck.java class. Notice that the mpHealth-3.0 feature still exists in `src/main/liberty/config/configDropins/overrides/generated-features.xml`. 

6. Go to the console where you started dev mode, press 'o' and Enter. This will re-scan all class files and generate an "optimized" list of features. Notice that `mpHealth-3.0` no longer exists in `src/main/liberty/config/configDropins/overrides/generated-features.xml` as it is no longer needed. 
