...
Notice that the `src/main/liberty/config/configDropins/overrides/generated-features.xml` file was updated and now contains the `mpHealth-3.1` feature.
```xml
<?xml version="1.0" encoding="UTF-8"?>
<server>
    <!--This file was generated by the Liberty Maven Plugin and will be overwritten on subsequent runs of the liberty:generate-features goal.
...
5. Go to the console where you started dev mode, press 'o' and Enter. This will re-scan all class files and generate an "optimized" list of features. Notice that the `mpHealth-3.1` feature no longer exists in `src/main/liberty/config/configDropins/overrides/generated-features.xml` as it is no longer needed.
