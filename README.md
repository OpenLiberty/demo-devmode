...
[Devc (dev mode for containers) demo](https://github.com/OpenLiberty/demo-devmode/tree/devc)

## Demo scenario

### Hot deployment with automatic generation of features

1. Clone this repo: `git clone https://github.com/OpenLiberty/demo-devmode.git`

2. Start dev mode:
    - Maven: `mvn liberty:dev`
    - Gradle: `gradle libertyDev`

    Notice that a new file was automatically created at `src/main/liberty/config/configDropins/overrides/generated-features.xml` containing the `cdi-3.0`, `mpMetrics-4.0` and `restfulWS-3.0` features and these features were subsequently installed.
    ```xml
...
<details>
    <summary>3. Create the src/main/java/io/openliberty/sample/system/SystemLivenessCheck.java class. Notice that the "mpHealth-4.0" feature is generated and installed. Changes are reflected in the http://localhost:9080/health endpoint.  </summary>
...
Notice that the `src/main/liberty/config/configDropins/overrides/generated-features.xml` file was updated and now contains the `mpHealth-4.0` feature.
```xml
<?xml version="1.0" encoding="UTF-8"?>
<server>
    <!--This file was generated by the Liberty Maven Plugin and will be overwritten on subsequent runs of the liberty:generate-features goal.
...
4. Delete the src/main/java/io/openliberty/sample/system/SystemLivenessCheck.java class. Notice that the `mpHealth-3.1` feature still exists in `src/main/liberty/config/configDropins/overrides/generated-features.xml`. Dev mode retains all the generated features in case other class files need them. To remove a feature, dev mode will need to re-scan all the class files (shown in the next step).

5. Go to the console where you started dev mode, press 'o' and Enter. This will re-scan all class files and generate an "optimized" list of features. Notice that the `mpHealth-4.0` feature no longer exists in `src/main/liberty/config/configDropins/overrides/generated-features.xml` as it is no longer needed.

6. Add the src/main/java/io/openliberty/sample/system/SystemLivenessCheck.java class back to your application. Notice that the `mpHealth-3.1` feature is once again generated. Changes are reflected in the http://localhost:9080/health endpoint.

<details>
    <summary>7. Create the src/main/java/io/openliberty/sample/system/SystemReadinessCheck.java class.  Changes are reflected in the http://localhost:9080/health endpoint. </summary>
...
8. Change the `io_openliberty_guides_system_inMaintenance` variable in `src/main/liberty/config/server.xml` to `true`.  Changes are reflected in the http://localhost:9080/health endpoint.  Undo this afterwards.
...
### Toggling automatic generation of features on/off

1. Go to the console where you started dev mode, press 'g' and Enter. This will toggle automatic generation of features off.

2. Pressing 'g' and Enter again will toggle automatic generation of features on. Notice that dev mode immediately runs the generate features goal/task.
